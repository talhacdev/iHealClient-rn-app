...
class EditVehicle extends React.Component {
  ...
  componentDidMount() {
    const {user, getStates, navigation, getMake, getModel} = this.props;
    this.yearsFun();

    const {phone_no, session} = user;
    const data = navigation.getParam('data');
    new Promise((rsl, rej) => {
      getStates(phone_no, session, rsl, rej);
    })
      .then(res => {
        res.map(item => {
          if (item.state_id == data.state_id) {
            this.setState({
              emirate: res,
              loader: false,
              ['selectedEmirate']: item.state_name,
              ['selectedEmirateid']: item.state_id,
              ['selectedMake']: data.car_make,
              ['selectedMakeid']: data.make,
              ['selectedModel']: data.model_name,
              ['selectedModelid']: data.model,
              plate_code: data.plate_code,
              plate_number: data.plate_no,
              selectedColor: data.color,
              mulkiya: data.mulkiya,
    ...
    new Promise((rsl, rej) => {
      ...
      .catch(err => Alert.alert('Sorry', err));
    new Promise((rsl, rej) => {
      getModel(phone_no, session, data.make, rsl, rej);
    })
      .then(res => {
        this.setState({
          model: res,
  ...
  handler = () => {
    ...
    if (!plate_code) {
      ...
      // Keyboard.dismiss()
      // let vehicle_data = {
      //     plate_code, plate_number, selectedColor,
      //     make: this.state['selectedMakeid'],
      //     model: this.state['selectedModelid'],
      //     state_id: this.state['selectedEmirateid'],
      //     mulkiya
      // }
      // navigation.navigate('Lisence', { vehicle_data, vehicle_id, })
      Keyboard.dismiss();
      this.setState({loader: true});
      const {user, updateVehicle, navigation} = this.props;
